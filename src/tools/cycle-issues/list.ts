...
export class ListCycleIssuesTools implements Tool {
  ...
  async execute(args: Record<string, unknown>): Promise<ToolResponse> {
    ...
    console.error(`[DEBUG] Cycle issues list tool called with args: ${JSON.stringify(args)}`);
    ...
    // Validate project ID and cycle ID
    if (!input.project_id) {
      return {
        isError: true,
        content: [{
    ...
    try {
      console.error(`[DEBUG] Calling cycleIssuesClient.list with workspace=${workspace_slug}, project=${input.project_id}, cycle=${input.cycle_id}`);
      const response = await this.cycleIssuesClient.list(
        workspace_slug,
        input.project_id,
        input.cycle_id
      );

      console.error(`[DEBUG] Successfully retrieved cycle issues response`);
      return {
        content: [{
          type: 'text',
      ...
      console.error(`[ERROR] Failed to list cycle issues: ${errorMessage}`);
      
      // Provide more helpful error message
      let detailedError = `Failed to list cycle issues: ${errorMessage}`;
      if (errorMessage.includes('404')) {
        detailedError += `\n\nPossible reasons for 404 error:\n` +
          ...
          `4. You might not have permission to access this cycle's issues`;
      }
