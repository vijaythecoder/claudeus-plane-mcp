...
export class GetCycleTool implements Tool {
  ...
  async execute(args: Record<string, unknown>): Promise<ToolResponse> {
    ...
    try {
      const response = await this.cyclesClient.getCycle(
        workspace_slug,
        input.project_id,
        input.cycle_id
      );
      ...
      console.error(`[ERROR] Failed to get cycle: ${errorMessage}`);
      
      // Provide more helpful error message
      let detailedError = `Failed to get cycle: ${errorMessage}`;
