...
export class GetIssueCommentTool implements Tool {
  ...
  async execute(args: Record<string, unknown>): Promise<ToolResponse> {
    ...
    console.error(`[DEBUG] Get issue comment tool called with args: ${JSON.stringify(args)}`);
    ...
    try {
      const response = await this.issueCommentsClient.getComment(
        workspace_slug,
        input.project_id,
        input.issue_id,
        input.comment_id
      ...
      console.error(`[DEBUG] Successfully retrieved issue comment`);
      return {
        content: [{
          type: 'text',
      ...
      console.error(`[ERROR] Failed to get issue comment: ${errorMessage}`);
      
      // Provide more helpful error message
      let detailedError = `Failed to get issue comment: ${errorMessage}`;
      if (errorMessage.includes('404')) {
        detailedError += `\n\nPossible reasons for 404 error:\n` +
          ...
          `5. You might not have permission to access this comment`;
      }
